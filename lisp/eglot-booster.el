...
(defun eglot-booster--wrap-contact (args)
  ...
  (let ((contact (nth 3 args))
        (def-args
         (if eglot-booster-io-only
             eglot-booster--boost-io-only
           eglot-booster--boost)))
    (cond
     ((and eglot-booster-no-remote-boost
  ...
  (cond
   (eglot-booster-mode
    ...
    (unless eglot-booster-io-only
      (advice-add
       'jsonrpc--json-read
       :around #'eglot-booster--jsonrpc--json-read))
    (advice-add
     'eglot--connect
     :filter-args #'eglot-booster--wrap-contact)
    (add-hook 'eglot-server-initialized-hook #'eglot-booster--init))
   (t
    (advice-remove
     'jsonrpc--json-read #'eglot-booster--jsonrpc--json-read)
    (advice-remove 'eglot--connect #'eglot-booster--wrap-contact)
    (remove-hook
     'eglot-server-initialized-hook #'eglot-booster--init))))
